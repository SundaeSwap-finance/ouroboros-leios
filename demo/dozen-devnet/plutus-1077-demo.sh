...
# Static protocol-params fixture, built by hand from this devnet's genesis
# files (shelley/alonzo/conway/dijkstra-genesis.json) instead of `cardano-cli
# query protocol-parameters`, which hangs forever against this node (bug #3
# in the task notes, still open as of 2026-09-14). This devnet boots directly
# into Dijkstra with no protocol update applied, so genesis-derived params ==
# live params.
#
# 46 keys, not 35: this $CLI binary's own pinned cardano-ledger (cabal.project,
# distinct from the devnet's consensus/ledger pin -- bug #3/#4 "pin
# divergence") decodes into the OLDER 46-field Dijkstra PParams shape and
# rejects a 35-field file with "key \"maxPledgeLeverage\" not found" --
# confirmed live 2026-09-14, first empirical confirmation of the pin-diff
# hypothesis. The 11 extra keys (maxPledgeLeverage, minPoolMargin, 5x leios*,
# maxEndorserBlock{ReferencesSize,TxsSize,ExecutionUnits},
# maxRefScriptSizePerEndorserBlock) are all present in dijkstra-genesis.json
# regardless, so no guessing was needed. See the task notes for the two
# fields that are best-effort mappings (utxoCostPerByte, protocolVersion), not
# independently verified against a live query.
PROTOCOL_PARAMS=/Users/pascal/git/github.com/LEIOS/input-output-hk/ouroboros-leios/demo/dozen-devnet/protocol-params-dijkstra.json
...
if ! FUNDING_TXIN=$(wait_for_new_utxo "$FUNDING_ADDR" "$FUNDING_TXIN" "$WORKDIR/funding-utxo-sanity.json"); then
...
if ! FUNDING_TXIN=$(wait_for_new_utxo "$FUNDING_ADDR" "$COLLATERAL_TXIN" "$WORKDIR/funding-utxo-valid.json"); then
...
echo "done. Check relay11/node.log (or bp1/node.log) for the txMeasureAlonzo/txMeasureDijkstra canary trace."
